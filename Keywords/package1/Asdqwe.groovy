...
class Asdqwe {
	...
	def randomInt(int length) {
		...
		return sb.toString(); }
	/**
	 * Set Email with random string
	 * @return format email with random string
	 */
	@Keyword
	def getEmailForRegistration() {
		return ('UT.' + randomString(7)) + '@automation.com'
	}

	/**
	 * Get label from wheel picker
	 * @return format with label for wheel picker
	 */
	@Keyword
	def getLabelFromPicker(String list) {
		//		List wheels = driver.findElements(MobileBy.className("XCUIElementTypePickerWheel"))
		//		List wheels = findTestObject(MobileBy.className('XCUIElementTypePickerWheel'))
		//		wheels.get(0).setValue(Label)

		for (int i = 0; i < list.length; i++) {
			MobileElement we = (MobileElement) driver.findElementByXPath("//XCUIElementTypePickerWheel["+(i+1)+"]");
			we.sendKeys(list[i]);
		}

		//			driver.findElement(MobileBy.className("XCUIElementTypePickerWheel")).setValue(list)
		//		Point p1=testCase.getDriver().findElement(By.xpath('XCUIElementTypePickerWheel')).getLocation();
		//		Dimension d1 = testCase.getDriver().findElement(By.xpath('XCUIElementTypePickerWheel')).getSize();
		//	if
